...
def get_files(cat, exts={".jpg",".jpeg",".png",".webp",".pdf"}):
    d = A/cat
    return [str(p) for p in sorted(d.iterdir()) if p.suffix.lower() in exts] if d.exists() else []

def get_media(cat):
    """Get images, pdfs, and videos from a category."""
    d = A/cat
    if not d.exists(): return []
    valid = {".jpg",".jpeg",".png",".webp",".pdf",".mp4",".mov",".avi",".webm"}
    return [str(p) for p in sorted(d.iterdir()) if p.suffix.lower() in valid]
...
def mime(path):
    ...
    m = {"jpg":"image/jpeg","jpeg":"image/jpeg","png":"image/png","webp":"image/webp","pdf":"application/pdf",
         "mp4":"video/mp4","mov":"video/quicktime","avi":"video/x-msvideo","webm":"video/webm"}
    return m.get(ext, "application/octet-stream")

def render_file(filepath):
    """Render a single file inline — image, PDF first page, or video."""
    p = Path(filepath)
    ext = p.suffix.lower()
    if ext in {".jpg",".jpeg",".png",".webp"}:
        ...
        d = b64(filepath)
        st.markdown(f'<div style="max-width:720px;margin:0 auto"><iframe src="data:application/pdf;base64,{d}" width="100%" height="500px" style="border:1px solid {BD};border-radius:12px"></iframe></div>', unsafe_allow_html=True)
...
# Skill bar colors (1-10), no red
BAR_COLORS = {
    1: "#94a3b8", 2: "#64748b", 3: "#0891b2", 4: "#0d9488",
    5: "#059669", 6: "#16a34a", 7: "#2563eb", 8: "#4f46e5",
    9: "#7c3aed", 10: "#8b5cf6"
}

def skill_bar(name, level):
    """Render a skill bar with color based on level (1-10)."""
    color = BAR_COLORS.get(level, AC)
    pct = level * 10
    st.markdown(f"""
    <div style="margin-bottom:.5rem">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
            <span style="font-family:'DM Mono',monospace;font-size:.73rem;color:{T3}">{name}</span>
            <span style="font-family:'DM Mono',monospace;font-size:.65rem;color:{T4}">{level}/10</span>
        </div>
        <div style="height:6px;background:{BARBG};border-radius:3px;overflow:hidden">
            <div style="width:{pct}%;height:100%;background:{color};border-radius:3px;transition:width .6s cubic-bezier(.34,1.56,.64,1)"></div>
        </div>
    </div>
...
.tr a{{color:{TX};text-decoration:none;border-bottom:1px dashed {AC};transition:color .2s}}.tr a:hover{{color:{AC}}}
...
# ═══════════════════════════════════════════════
# TABS — anchor tabs for Experience/Education/Products route to sections on About Me page
# ═══════════════════════════════════════════════
tabs = st.tabs(["About Me", "Academics", "Certificates", "Projects & Videos", "Contact"])


# ╔═══════════════════════════════════════════════════════════╗
# ║  ABOUT ME  (Hero + Skills + Experience + Education + Products)
# ╚═══════════════════════════════════════════════════════════╝
with tabs[0]:
    ...
    with cp:
        pf = get_files("profile", {".jpg",".jpeg",".png",".webp"})
        if pf:
    ...
    with ci:
        ...
        <div class="hero au">
            ...
            <div class="hero-bio">Lead R&D Engineer and TUM graduate with 5+ years across Robert Bosch and high-growth startups. Owns embedded and ML-based healthcare systems end-to-end — from firmware and edge ML to cloud inference pipelines.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    for i, (cat, sk_list) in enumerate(skills_with_levels.items()):
        with c1 if i % 2 == 0 else c2:
            ...
            for name, level in sk_list:
                skill_bar(name, level)
            st.markdown("")
    ...
    exps = [
        ...
        ("Embedded Software Developer","AmberFlux EdgeAI · Hyderabad","Nov 2020 – Jan 2021",
         ["Integrated sensors on RPi, STM32 and Xtensa for real-time positioning",
    ...
    prods = [
        ("Health Band — O-Health",
         "Production-grade wearable with custom sensors, BLE, and cloud ML. Collaborating with MIT CSAIL.",
         ["Hardware","Firmware","ML","BLE","Healthcare"], None),
        ("ASR Engine for Hindi",
         "Edge-optimized ASR. 40% WER improvement, 50% latency reduction. Deployed in 30,000+ clinical consultations worldwide.",
         ["ASR","Edge ML","Compression","NLP"], None),
        ("Medical RAG Pipeline",
         "Hallucination-free medical info extraction via cross-LLM consensus.",
         ["RAG","LLM","Healthcare","NLP"], None),
        ("Retrofittable E-Bike Auto Shifter",
         "IoT-enabled rear-derailleur shifting with mode-based transmission. IEEE ICCE 2024 Best Presentation.",
         ["IoT","Mechatronics","Sensors","IEEE"], "https://ieeexplore.ieee.org/document/10444469"),
        ("SecOC Tool — Bosch",
         "Automated ECU data encryption for CAN bus.",
         ["Automotive","Security","CAN","CAPL"], None),
        ("Autonomous Mini Vehicle — TUM",
         "Lane-free traffic control with V2V and sensor fusion.",
...
with tabs[1]:
    ...
    <div class="tc au d1"><div class="th_"><div>
        ...
        <div class="tco">Master's Thesis — TU München</div></div><span class="tdt">Feb – Oct 2024</span></div>
    ...
    <div class="tc au d2"><div class="th_"><div>
        ...
        <div class="tco">Würth Elektronik eiSoS — TU München</div></div><span class="tdt">2022 – 2024</span></div>
    ...
    # 3D Solar System
    st.markdown(f"""
    <div class="tc au d3"><div class="th_"><div>
        <div class="tr"><a href="#solar-docs">3D Manoeuverable Solar Retrofittable System</a></div>
        <div class="tco">Academic Project</div></div><span class="tdt">Project</span></div>
    <div class="td">Design and development of a 3D manoeuverable solar panel system that can be retrofitted onto existing structures for optimal sun tracking and energy harvesting.</div>
    <div class="tsk"><span class="sk">Solar Energy</span><span class="sk">Mechatronics</span><span class="sk">Retrofittable</span><span class="sk">3D Tracking</span></div>
    <div><a class="db" href="#solar-docs">📄 PDF / Presentation</a></div></div>
    """, unsafe_allow_html=True)

    # WSN Data Extraction
    st.markdown(f"""
    <div class="tc au d4"><div class="th_"><div>
        <div class="tr"><a href="#wsn-docs">Wireless Sensor Network for Data Extraction</a></div>
        <div class="tco">Academic Project</div></div><span class="tdt">Project</span></div>
    <div class="td">Development of a wireless sensor network system for environmental data extraction and processing using distributed sensor nodes.</div>
    <div class="tsk"><span class="sk">WSN</span><span class="sk">Sensor Networks</span><span class="sk">Data Extraction</span><span class="sk">Embedded</span></div>
    <div><a class="db" href="#wsn-docs">📄 PDF / Presentation</a></div></div>
    """, unsafe_allow_html=True)

    # Publications WITH YEAR
    ...
    for t,yr,v,u,d in [
        ...
        st.markdown(f'<div class="pc af"><div class="pt"><a href="{u}" target="_blank">{t}</a></div><div class="pv">{v} · {yr}</div><div class="pl"><a href="{u}" target="_blank">📄 {u}</a></div><div class="pd_">{d}</div></div>', unsafe_allow_html=True)

    # Conferences — TI Innovation removed
    ...
    st.markdown("---")
    st.markdown(f'<div id="solar-docs"></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="stitle au">📄 3D Manoeuverable Solar System — Document</div>', unsafe_allow_html=True)
    embed_doc("solar_system.pdf","3D Solar System")

    st.markdown("---")
    st.markdown(f'<div id="wsn-docs"></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="stitle au">📄 Wireless Sensor Network — Document</div>', unsafe_allow_html=True)
    embed_doc("wsn_data_extraction.pdf","WSN Data Extraction")
