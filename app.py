...
def pdf_to_png_b64(filepath, page=0, dpi=150):
    """Convert PDF page to PNG base64. Returns (b64_string, total_pages) or (None, 0)."""
    try:
        import fitz
...
def render_file(filepath):
    ...
    if ext in {".jpg",".jpeg",".png",".webp"}:
        ...
        if img_b64:
            ...
            st.download_button(f"⬇ Download {p.name}", data, file_name=p.name, mime="application/pdf")
...
def skill_bar(name, level):
    ...
    st.markdown(f'<div style="margin-bottom:.45rem"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px"><span style="font-family:DM Mono,monospace;font-size:.72rem;color:{T3}">{name}</span><span style="font-family:DM Mono,monospace;font-size:.63rem;color:{T4}">{level}/10</span></div><div style="height:5px;background:{BARBG};border-radius:3px;overflow:hidden"><div style="width:{level*10}%;height:100%;background:{color};border-radius:3px;transition:width .6s cubic-bezier(.34,1.56,.64,1)"></div></div></div>', unsafe_allow_html=True)
...
.ec{{background:{CD};border:1px solid {BD};border-radius:14px;padding:1.2rem;margin-bottom:.8rem;transition:all .3s}}
...
with tabs[0]:
    ...
    tl_path = A/"docs"/TL_IMG
    if tl_path.exists():
        st.markdown(f'<div style="max-width:900px;margin:0 auto;border-radius:14px;overflow:hidden;border:1px solid {BD}"><img src="data:image/png;base64,{b64(str(tl_path))}" style="width:100%;display:block"/></div>', unsafe_allow_html=True)
    else:
        st.info("Timeline image not found. Run the generation script or add `timeline_dark.png` / `timeline_light.png` to `assets/docs/`.")

    # ── SKILLS ──
    st.markdown(f'<div class="stitle af d4">Core skills</div>', unsafe_allow_html=True)
    skills_with_levels = {"Embedded & IoT":[("Edge ML",9),("Arduino",8),("Raspberry Pi",8),("STM32",9),("Firmware",9),("IoT Security",7),("CAN",8),("V2V",7)],"AI / ML":[("ASR",8),("NLP",8),("RAG",7),("Model Compression",8),("TensorRT",7),("GPU Accel.",7),("Computer Vision",6)],"Programming":[("Python",9),("C",8),("C++",8),("Java",6),("CAPL",7),("ROS",6)],"Platforms":[("Linux/RHEL",8),("CUDA",7),("NVIDIA Jetson",8),("Docker",7),("Git",9)]}
    c1,c2 = st.columns(2)
    ...
    for idx,(role,co,dt,bul,sk) in enumerate([
        ("Lead R&D Product Engineer","O-Health · Bangalore & Boston","Jan 2025 – Present",["Improved ASR accuracy — reduced WER by 40% for Standard Hindi on edge models","Reduced inference latency by 50% via knowledge distillation, quantization, weight pruning","Built medical RAG pipeline with hallucination-free outputs via cross-LLM consensus","Optimized GPU inference on H200 clusters — CUDA, TensorRT, async multi-threading","Led end-to-end health-band development with MIT CSAIL"],["ASR","NLP","RAG","Linux","Edge ML","GPU Accel.","Health-tech"]),
        ("Werkstudent Embedded IoT Engineer","Würth Elektronik eiSoS · Munich","May 2022 – Jan 2025",["Optimized AT-command interfaces and multi-threaded buffer management","Designed low-level LTE-M / NB-IoT firmware drivers with robust state machines","Ensured reliable high-frequency IoT data transmission under variable conditions"],["LTE-M","NB-IoT","Firmware","Multithreading","ARM","Edge ML"]),
        ("Embedded Software Engineer","Robert Bosch · Bangalore","Jan 2021 – Sep 2021",["Implemented ECU security algorithms per customer specifications","Developed SecOC tool for secure data transmission between ECUs over CAN bus"],["ECU Security","CAN","CAPL","CANoe","XCP"]),
    ...
    prods=[("Health Band — O-Health","Production-grade wearable with custom sensors, BLE, and cloud ML. Collaborating with MIT CSAIL.",["Hardware","Firmware","ML","BLE","Healthcare"],None),("ASR Engine for Hindi","Edge-optimized ASR. 40% WER improvement, 50% latency reduction. Deployed in 30,000+ clinical consultations worldwide.",["ASR","Edge ML","Compression","NLP"],None),("Medical RAG Pipeline","Hallucination-free medical info extraction via cross-LLM consensus.",["RAG","LLM","Healthcare","NLP"],None),("Retrofittable E-Bike Auto Shifter","IoT-enabled rear-derailleur shifting with mode-based transmission. IEEE ICCE 2024 Best Presentation.",["IoT","Mechatronics","Sensors","IEEE"],"https://ieeexplore.ieee.org/document/10444469"),("SecOC Tool — Bosch","Automated ECU data encryption for CAN bus.",["Automotive","Security","CAN","CAPL"],None),("Autonomous Mini Vehicle — TUM","Lane-free traffic control with V2V and sensor fusion.",["Autonomous","V2V","ROS","Sensor Fusion"],None)]
...
with tabs[1]:
    ...
    for title,venue,date,desc,skills,doc_id,btns in [
        ...
        ("Cellular IoT Cloud Connectivity","Würth Elektronik eiSoS — TU München","2022 – 2024","Designed a plug-and-play system enabling seamless communication between the sensor module and cellular Adrastea-I, transmitting data to the cloud in secure mode via digital certificates in both LTE-CAT-M and NB-IoT modes with PSM power saving.",["LTE-M","NB-IoT","PSM","Cloud IoT","Digital Certificates","Adrastea-I"],"cellular-iot-docs",[("📄 PDF","cellular-iot-docs")]),
        ("3D Manoeuverable Solar Retrofittable System","Academic Project","Project","Design and development of a 3D manoeuverable solar panel system retrofittable onto existing structures for optimal sun tracking and energy harvesting.",["Solar Energy","Mechatronics","Retrofittable","3D Tracking"],"solar-docs",[("📄 PDF / Presentation","solar-docs")]),
        ("Wireless Sensor Network for Data Extraction","Academic Project","Project","Development of a wireless sensor network system for environmental data extraction and processing using distributed sensor nodes.",["WSN","Sensor Networks","Data Extraction","Embedded"],"wsn-docs",[("📄 PDF / Presentation","wsn-docs")]),
    ]:
    ...
    # Publications — NO duplicate years
    ...
    for t,v,u,d in [
        ...
        ("Remote Monitoring Robot with Voice Control and Image Analysis","Scopus — IJAST 2020","http://sersc.org/journals/index.php/IJAST/article/view/21218","Surveillance robot with image processing, voice control, and MQTT."),
    ...
    # Documents
    st.markdown("---")
    for anchor,title,fname,label in [
        ("lane-free-docs","Lane-Free Traffic Control","lane_free_thesis.pdf","Thesis"),
    ]:
    ...
    for anchor,title,fname,label in [
        ("cellular-iot-docs","Cellular IoT Cloud Connectivity","cellular_iot.pdf","Cellular IoT"),
        ("solar-docs","3D Solar Retrofittable System","solar_system.pdf","3D Solar System"),
        ("wsn-docs","Wireless Sensor Network","wsn_data_extraction.pdf","WSN Data Extraction"),
    ]:
        st.markdown(f'---<div id="{anchor}"></div><div class="stitle au">📄 {title}</div>',unsafe_allow_html=True)
        render_doc(fname,label)
